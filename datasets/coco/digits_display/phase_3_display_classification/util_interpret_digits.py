...
if drone_debug:
    #drone_debug_run = 'temp_' + str()
    drone_debug_run = 'to_delete'
    drone_debug_path = os.path.join('.', drone_debug_run)
    if not os.path.exists(drone_debug_path):
        os.mkdir(drone_debug_path)

    for i in range(4):
        path = os.path.join(drone_debug_path, str(i))
        if not os.path.exists(path):
            os.mkdir(path)

    cnt = [0 for i in range(4)]

    def drone_debug_save(image, step):
        path = os.path.join(drone_debug_path, '%d/%d.png' % (step, cnt[step]))
...
high_white_display = (100, 100, 255)
...
def binarize_image(image):
    ...
    sure_display = cv2.inRange(
        HSV, low_white_display, high_white_display)  # sensor green
    return sure_display
...
def expand_image_to_given_size(image, num_output_pixels):
    ...
    if y > num_output_pixels or x > num_output_pixels:
        raise Exception('increase num_output_pixels to ' +
                        str(x) + ' or ' + str(y))
...
def extract_digits_from_display(image, num_pixels_output_image, num_pixels_each_digit):
    ...
    if image_h != image_w or num_pixels != image_w:
        resized_image = cv2.resize(
            image, (num_pixels, num_pixels), interpolation=cv2.INTER_CUBIC)
    else:
        resized_image = image
    ...
    all_indices[1] = [first_digits_end_x, 0,
                      second_digits_end_x, half_num_pixels]
    all_indices[2] = [0, half_num_pixels, first_digits_end_x, num_pixels-1]
    all_indices[3] = [first_digits_end_x, half_num_pixels,
                      second_digits_end_x, num_pixels-1]

    for i in range(4):
        cropped_image = crop_image(resized_image, all_indices[i])
        #print('cropped_image', cropped_image.shape)
        digit_image = expand_image_to_given_size(
            cropped_image, num_pixels_each_digit)
...
def resize_template(image, template):
    ...
    # print("template ", h_template, w_template)
    if h_template != h:
        resizing_factor = float(target_h) / float(h_template)
...
def step_2_find_display_bounding_box(binary_image, image):
    ...
    # stores all points, from all contours
    all_points = np.zeros((num_points, 1, 2), dtype=int)
    index = 0
    for c in contours:
...
def step_3_rotate_display(binary_image, rectangle, image, percent_template):
    ...
    new_rectangle = step_2_find_display_bounding_box(
        rotated_binary_image, rotated_image)
    ...
    # cv2.imshow('DIEGO DEBUG 1', cropped_binary_image)
    # cv2.waitKey(0)
    # cv2.imshow('DIEGO DEBUG 2', percent_template)
    # cv2.waitKey(0)
    ...
    resized_percent_template = resize_template(
        cropped_binary_image, percent_template)
    angle_max = get_best_rotation_angle(
        cropped_binary_image, resized_percent_template, angles)
...
def step_all_from_camera_to_rotated_display(original_image, percent_template):
    ...
    # Debugs
    if drone_debug:
        drone_debug_save(binary_image, 1)
    if show_plots_to_debug:
    ...
    # Debugs
    if drone_debug or show_plots_to_debug:
        image2 = deepcopy(image)
        box = cv2.boxPoints(rectangle)
        box = np.int0(box)
        cv2.drawContours(image2, [box], 0, (0, 0, 255), 2)
    if drone_debug:
        drone_debug_save(image2, 2)
    if show_plots_to_debug:
        cv2.imshow('step_all_display found via contours', image2)
        cv2.waitKey(0)

    cropped_binary_image, cropped_image = step_3_rotate_display(
        binary_image, rectangle, image, percent_template)

    # Debugs
    if drone_debug:
        drone_debug_save(cropped_binary_image, 3)
    if show_plots_to_debug:
